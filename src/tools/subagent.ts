...
export const spawnSubagentTool: Tool = {
  name: "spawn_subagent",
  description:
    "Spawn a sub-agent to handle a task in the background. The sub-agent runs autonomously with a restricted tool set.",
  parameters: {
    type: "object",
    properties: {
      ...
      max_iterations: {
        type: "number",
        description: "Max tool iterations (default: 20)",
      },
    },
  ...
  async execute(
    input: { name: string; prompt: string; max_iterations?: number },
    ctx: ToolContext,
  ): Promise<ToolResult> {
    ...
    if (running.length >= MAX_CONCURRENT) {
      return {
        output: `Max concurrent sub-agents (${MAX_CONCURRENT}) reached`,
        isError: true,
      };
    }

    const id = nextId++;
    const entry: { id: number; name: string; status: string; result?: string } =
      { id, name: input.name, status: "running" };
    ...
    (async () => {
      try {
        const rawResult = await processMessage(input.prompt, {
          ...
          config: {
            ...ctx.config,
            max_tool_iterations: input.max_iterations || 20,
          },
          registry: ctx.registry!,
        });
        const result =
          typeof rawResult === "string" ? rawResult : "Interrupted";
    ...
    return {
      output: `Sub-agent #${id} "${input.name}" spawned. Use list_subagents to check status.`,
    };
  },
};
