...
export const sendMessageTool: Tool = {
  name: "send_message",
  description:
    "Send a message to a specific chat. Use for proactive updates, cross-chat messaging, or intermediate responses.",
  parameters: {
    type: "object",
    properties: {
      channel: {
        type: "string",
        description:
          "Channel name (e.g., 'discord', 'slack', 'imessage', 'signal')",
      },
      chat_id: {
        type: "string",
        description: "External chat ID for the target channel",
      },
  ...
  async execute(
    input: { channel?: string; chat_id?: string; text: string },
    ctx: ToolContext,
  ): Promise<ToolResult> {
    if (!channelRegistryRef)
      return { output: "Channel registry not available", isError: true };

    const targetChannel = input.channel || ctx.channel;
    const adapter = channelRegistryRef.get(targetChannel);
    if (!adapter)
      return { output: `Channel not found: ${targetChannel}`, isError: true };
